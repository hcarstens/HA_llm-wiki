...
### Estimating the skeleton ceiling

We can bound this because CT IS a skeleton, and we tested it on all three task types:

| Path | Creative task (C2) | Forecasting task (F3 with data) |
|---|---|---|
| Skeleton (CT) alone | 1446 | 1505 |
| Skeleton (CT) + best heuristic | ~1572 (Invention, C1 re-judge) | not tested in composition |
| **Program alone** | **1563 (GI)** | **1602 (FS)** |

The reasoning campaigns show what iterative skeleton composition maxes out at:

```
CT skeleton alone:             ~1538
CT + best single heuristic:   ~2158 (CT + Economics, +620)
Best compound after 6 rounds:  1830 (Decisive Calibration)
```

The skeleton path gained ~292 Elo over CT alone to reach DC — roughly a 19% lift from iterative composition. Applying that ratio to hypothetical creativity/forecasting skeletons:

| Hypothetical skeleton path | Estimated ceiling | Program (actual) | Skeleton wins? |
|---|---|---|---|
| Creativity skeleton × 1.19 | ~1720 | GI = 1654 | Maybe — but after 6 rounds |
| Forecasting skeleton × 1.19 | ~1790 | FS = 1602 | Probably — but after 6 rounds |

### Why skeletons probably wouldn't have worked for creativity

The deeper problem: **what would a creativity skeleton say?**

```
Hypothetical creativity skeleton:
"Novelty, Surprise, Coherence, Value, Constraint, Imagination, Emotion"
```

That's the creativity rubric dimensions rephrased as a prompt. It tells the model *what qualities to exhibit* but not *how to produce them*.

CT works as a skeleton because reasoning IS a set of qualities — telling a model to "be precise and logical" is a sufficient instruction for analytical tasks. The model already knows how to be precise; the label activates the capability.

Creativity is different. Telling a model to "be novel" doesn't produce novelty the way telling it to "be precise" produces precision. Creativity requires a process — "violate one assumption, test if it's been done before, take the most surprising candidate and elaborate." These are things the model can DO, not things it can BE. GI's power comes from executable steps, not aspirational labels.

### Why the program path is more efficient

| Path | Steps to reach end state | Method |
|---|---|---|
| **Skeleton → compound** (reasoning) | 6 iterative rounds (R1-R6), ~1500 experiments, ~$50 | Discovery by iteration — test, judge, refine, repeat |
| **Program by design** (creativity, forecasting) | 1 design step + 2-3 validation campaigns | Design by derivation — apply HA operators (⊕, π, ¬) to source heuristics |

The Heuristic Algebra shortcut: GI was derived in one step via π₅(Im ⊕ Cr) using the ⊕_inn operator. FS was practitioner-designed from forecasting domain knowledge. Both achieved their end state without iterative search.

The skeleton path (R1-R6) was iterative search — test 20 heuristics, find the best compositions, combine them, test again. Each round narrowed the search space. Six rounds to converge. The algebra lets you derive the converged result directly from the axiom sources.

### Probability estimates

| Scenario | Probability | Reasoning |
|---|---|---|
| Creativity skeleton + 6 rounds beats GI | ~20% | Would need to discover creative process through iteration — but GI was designed from that process directly |
| Forecasting skeleton + 6 rounds beats FS | ~30% | Slightly higher — forecasting has more "quality" aspects that skeleton labels can capture |
| Either skeleton path is more efficient than direct program design | ~5% | The program was designed in one step; the skeleton path took 6 rounds |

### The meta-insight

The R1-R6 reasoning progression was the transition from skeleton to program: CT → CT+heuristic → compound → Decisive Calibration (a program). The skeleton path ENDS at a program — it's discovery by iterative search of what the algebra can derive in one step.

GI and FS are what CT *would have become* if Heuristic Algebra had existed at the start of the reasoning campaign. The algebra doesn't replace the skeleton path — it shortcuts it.
